...
import pyswitch
from pyswitch.api.community.events import CommunityEvent
from pyswitch.api.chat.events import ChatEvent
from pyswitch.bots import BotContext, Decorators, BaseHandler
...
class BotApp(App, Decorators):
    ...
    def __init__(
        ...
        self._user_type = pyswitch.bots.Bot
        ...
        self._register_commands: List[pyswitch.bots.RegisterCommand] = []
        self._bot_description = bot_description
        self.auto_update_bot = auto_update_bot

    @property
    def bot(self) -> "pyswitch.bots.Bot":
        return self.user
    ...
    def register_command(
        self, command: pyswitch.bots.RegisterCommand | List[pyswitch.bots.RegisterCommand]
    ) -> "BotApp":
        if self._running:
            raise pyswitch.SwitchError("Cannot register commands after the app has started")
    ...
    async def _validate_token(self):
        ...
        if not isinstance(self.user, pyswitch.Bot):
            raise pyswitch.SwitchError("Invalid token")

        if not self.user.is_bot:
            raise pyswitch.SwitchError("Invalid token (not a bot)")
